...
fn run(cnt:&[usize;4],name:&str,ty:usize){
    for t in 0..ty{
        eprintln!();
        eprintln!("{} / {} : ",t,ty);
        ...
        if let Some(ans)=solve(&cnt,&plan,&obs){
            if let Some((v,g,a))=adjust_graph_triple(&ans){
                let f=guess(v,&g,&a);
                if f{
                    eprintln!("correct");
                    break;
...
fn adjust_graph_triple(map:&Map)->Option<(usize,[[usize;6];N*3],[usize;N*3])>{
    let start=map.v;
    let g=&map.g;
    let a=&map.a;
    
    let g=restore_graph(g);
    let (cpp_start,cpp_end,tour_es)=solve_cpp(&g,start);
    let tour_vs={
        let a=(0..N).filter(|&i|i!=start).collect::<Vec<_>>();
        solve_tsp(&g,start,&a)
    };
    ...
    let mut cycle=vec![];
    for v in 0..N{
        if let Some(c)=find_minimum_cycle(&g,v){
            if cycle.is_empty() || cycle.len()>c.len(){
                cycle=c;
                cycle_start=v;
            }
        }
    ...
    let cycle=cycle.into_iter().map(|d|Move(d)).collect::<Vec<_>>();

    let find_path=|start:usize,goal:usize|->Vec<usize>{
    ...
    // let mut shuf=[[[!0;3];6];N];
    // for i in 0..N{
    //     for j in 0..6{
    //         let perm=[[0,1,2],[0,2,1],[1,0,2],[1,2,0],[2,0,1],[2,1,0]].choice();
    //         shuf[i][j]=perm;
    ...
    //         if (i,j)==(a,b){
    //             shuf[i][j]=[[0,1,2],[0,2,1],[1,0,2],[2,1,0]].choice();
    //         }
    //     }
    // }
    // let gstart=start+N*rnd::get(3);
    
    // let mut fail=false;
    // let mut explore=|plan:&[Act]|{
    //     let mut v=gstart;
    //     let mut a={
    //         let mut new=[!0;N*3];
    //         for i in 0..N{
    ...
    let mut plan=vec![Change(a[v]^1)];
    let mut rev=vec![Change(a[v]^2)];

    for &d in &tour_vs{
        plan.push(Move(d));
        rev.push(Move(g[v][d].1));

        v=g[v][d].0;
        if !seen[v]{
            seen[v]=true;
            plan.push(Change(a[v]^1));
            rev.push(Change(a[v]^2));
        }
    }
    assert!(seen==[true;N]);

    for &d in &find_path(v,cycle_start){
        plan.push(Move(d));
        rev.push(Move(g[v][d].1));
        v=g[v][d].0;
    }
    assert!(v==cycle_start);
    plan.extend(&cycle);
    plan.extend(rev.iter().rev());

    v=start;
    plan.extend(find_path(v,cpp_start).iter().map(|&d|Move(d)));
    v=cpp_start;
    ...
    for &d in &tour_es{
        let (nv,nd)=g[v][d];
        plan.push(Move(d));
        rev.push((Move(nd),NULL));
        
        if !seen[v][d]{
            seen[v][d]=true;
            seen[nv][nd]=true;
            info.push((plan.len()-1,plan.len(),(v,d),(nv,nd)));
            rev.last_mut().unwrap().1=((nv,nd),(v,d));
        }
        v=nv;
    }
    assert!(seen==[[true;6];N]);
    ...
    for &d in &find_path(v,cycle_start){
        plan.push(Move(d));
        rev.push((Move(g[v][d].1),NULL));
        v=g[v][d].0;
    }
    assert!(v==cycle_start);
    plan.extend(&cycle);
    
    let mut info_rev=vec![];
    for &(act,item) in rev.iter().rev(){
        plan.push(act);
        if item!=NULL{
            info_rev.push((plan.len()-1,plan.len(),item.0,item.1));
        }
    }
    ...
    let mut newg=[[!0;6];N*3];
    for &(i0,i1,(v0,d0),(v1,d1)) in &info{
        let &(j1,j0,_,_)=info_rev.iter().find(|&t|t.2==(v1,d1) && t.3==(v0,d0)).unwrap();
        ...
        // eprintln!("{:?}",shuf[v0][d0]);
        // eprintln!("{:?}",shuf[v1][d1]);
        // eprintln!("{}({}/{}) -> {}({}/{})",o0,vs[i0]%N,vs[i0]/N,o1,vs[i1]%N,vs[i1]/N);
        // eprintln!("{}({}/{}) -> {}({}/{})",p0,vs[j0]%N,vs[j0]/N,p1,vs[j1]%N,vs[j1]/N);
        // assert!(o0!=p0 && o1!=p1);
        
        if !(o0==a[v0] || o0==a[v0]^1 || o0==a[v0]^2){
            return None;
        }
        if !(o1==a[v1] || o1==a[v1]^1 || o1==a[v1]^2){
            return None;
        }
        if !(p0==a[v0] || p0==a[v0]^1 || p0==a[v0]^2){
            return None;
        }
        if !(p1==a[v1] || p1==a[v1]^1 || p1==a[v1]^2){
            return None;
        }

        // assert!(o0==a[v0] || o0==a[v0]^1 || o0==a[v0]^2);
        // assert!(o1==a[v1] || o1==a[v1]^1 || o1==a[v1]^2);
        // assert!(p0==a[v0] || p0==a[v0]^1 || p0==a[v0]^2);
        // assert!(p1==a[v1] || p1==a[v1]^1 || p1==a[v1]^2);
        ...
        let vv0=v0%N;
        let vv1=v1%N;
        newg[vv0][d0]=vv1+get(0)*N;
        newg[vv1+get(0)*N][d1]=vv0;

        newg[vv0+N][d0]=vv1+get(1)*N;
        newg[vv1+get(1)*N][d1]=vv0+N;

        newg[vv0+N*2][d0]=vv1+get(2)*N;
        newg[vv1+get(2)*N][d1]=vv0+N*2;
    }
